...
public class ResolverGenerator : ISourceGenerator
{
    ...
    public void Execute(GeneratorExecutionContext context)
    {
        if (context.SyntaxContextReceiver is not ResolverSyntaxReceiver receiver)
            return;
        var resolverInterface = context.Compilation.GetTypeByMetadataName("ACCR.ACCR.IResolver");
        var aspectInterface = context.Compilation.GetTypeByMetadataName("ACCR.ACCR.IAspect");
        var resolverEventHandler = context.Compilation.GetTypeByMetadataName("ACCR.ACCR.ResolverEventHandler");
        var observerInterface = context.Compilation.GetTypeByMetadataName("ACCR.Utils.IObserver");
    ...
    private static string GenerateActionInitializations(List<IMethodSymbol> actions)
    {
    ...
    private class ResolverSyntaxReceiver : ISyntaxContextReceiver
    {
        ...
        public void OnVisitSyntaxNode(GeneratorSyntaxContext context)
        {
            ...
            if (!classSymbol.GetAttributes()
                    .Any(ad => ad.AttributeClass?.ToDisplayString() == "ResolverAttribute")) return;
            {
