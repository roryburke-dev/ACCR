...
public class CoordinatorGenerator : ISourceGenerator
{
    ...
    public void Execute(GeneratorExecutionContext context)
    {
        ...
        List<IFieldSymbol> _containers = [];
        foreach (var field in receiver.Containers.Values.SelectMany(containers => containers.Where(field => !_containers.Contains(field))))
        {
            _containers.Add(field);
        }
        ...
        var generatedEnum = $$"""
                              ...
                              namespace ACCR.Generated
                              {
                                  ...
                                  {
                                      {{GenerateContainerEnum(_containers)}}
                                  }
                              }
                              """;
        
        context.AddSource("ContainerEnum.g.cs", generatedEnum);
        ...
        foreach (var classSymbol in receiver.Coordinators)
        {
            ...
            var generatedCode = $$"""
                                  ...
                                  namespace {{classSymbol.ContainingNamespace?.ToDisplayString()}}
                                  {
                                      ...
                                      {
                                          public Dictionary<int, {{containerInterface}}> Containers { get; set; }
                                          ...
                                          public void Initialize(Dictionary<int, {{containerInterface}}> containers)
                                          {
                                              Containers = containers;
                                              States = new Dictionary<Type, {{stateInterface}}>();
                                              {{GenerateContainerInitializations(classSymbol.Name, containers)}}
                                              {{GenerateResolverInitializations(resolvers)}}
                                              {{GenerateStateInitializations(states)}}
                                          }
                                          
                                          public void AddContainer(int id, {{containerInterface}} container)
                                          {
                                          ...
                                          public T GetContainer<T>(int id) where T : {{containerInterface}}
                                          {
                                          ...
                                          public Dictionary<int, {{containerInterface}}> GetAllContainers() 
                                          {
                                              return Containers;                  
                                          }
                                          ...
                                          public {{infoInterface}}<T> OnNotify<T>({{infoInterface}}<T> info)
                                          {
                                              return info switch
                                              {
                                                  ...
                                                  {{infoInterface}}<float>                          floatInfo           => OnNotify(floatInfo)          as {{infoInterface}}<T>,
    ...
    private static string GenerateContainerEnum(List<IFieldSymbol> containers)
    {
        var builder = new StringBuilder();
        var firstLine = true;
        List<string> containerEnum = [];
        foreach (var field in containers.Where(field => !containerEnum.Contains(field.Name.Split('.').LastOrDefault()?.Replace("_", "").ToUpper())))
        {
            if(!firstLine) 
                builder = Utils.Indent(builder, 2);
            else 
                firstLine = false;
            containerEnum.Add(field.Name.Split('.').LastOrDefault()?.Replace("_", "").ToUpper());
    ...
    private static string GenerateContainerInitializations(string className, List<IFieldSymbol> containers)
    {
        var builder = new StringBuilder();
        var firstLine = true;
        foreach (var field in containers)
        {
            ...
            if(!firstLine) 
                ...
                firstLine = false;
            builder.AppendLine($"{fieldName} = ({field.Type})Containers[(int)Container.{field.Name.Split('.').LastOrDefault()?.Replace("_", "").ToUpper()}];");
            builder = Utils.Indent(builder, 3);
            builder.AppendLine($"{fieldName}.Initialize((int)Container.{field.Name.Split('.').LastOrDefault()?.Replace("_", "").ToUpper()});");
        }
        return builder.ToString();
    }
