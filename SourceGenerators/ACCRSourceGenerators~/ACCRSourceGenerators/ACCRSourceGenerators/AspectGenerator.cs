...
public class AspectGenerator : ISourceGenerator 
{
    ...
    private class AspectSyntaxReceiver : ISyntaxContextReceiver
    {
        ...
        {
            if (context.Node is not StructDeclarationSyntax  { AttributeLists.Count: > 0 } structDeclarationSyntax)
                return;
            ...
            if (structSymbol.GetAttributes().Any(ad => ad.AttributeClass?.ToDisplayString() == "AspectAttribute"))
            {
                ...
                {
                    if (field.GetAttributes().Any(ad => ad.AttributeClass?.ToDisplayString() == "SOAttribute"))
                    {
                        Fields[structSymbol].Add(field);
                    }
                }
            }
