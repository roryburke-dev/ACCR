...
public class ContainerGenerator : ISourceGenerator
{
            ...
            var generatedCode = $$"""
                                  ...
                                  namespace {{classSymbol.ContainingNamespace?.ToDisplayString()}}
                                  {
                                      public partial class {{classSymbol.Name}} : {{containerInterface}}
                                      {
                                          ...
                                          public Dictionary<string, {{aspectInterface}}> Aspects {get; set;}
                                          {{GenerateAspectScriptableObjectFields(aspects)}}
                                          
                                          public void Initialize(string id)
                                          {
                                              Id = id;
                                              Aspects = new Dictionary<string, {{aspectInterface}}>();
                                              {{GenerateAspectFieldInitializations(aspects)}}
                                          }
                                          
                                          public T GetAspect<T>(string id) where T : struct, {{aspectInterface}}
                                          {
                                              return Aspects.ContainsKey(id) ? (T)Aspects[id] : default(T);
                                          }
                                          
                                          public bool GetAspect<T>(string id, out T aspect) where T : struct, {{aspectInterface}}
                                          {
    ...
    private static string GenerateAspectUpdaters(List<IFieldSymbol> aspects)
    {
        ...
        foreach (var field in aspects)
        {
            ...
            builder = Utils.Indent(builder, 3);
            builder.AppendLine($"Aspects[\"{field.Name.Split('.').LastOrDefault()?.Replace("_", "").ToUpper()}\"] = value;");
            builder = Utils.Indent(builder, 2);
            builder.AppendLine("}");
        }
        return builder.ToString();
    ...
    private static string GenerateAspectFieldInitializations(List<IFieldSymbol> aspects)
    {
        ...
        foreach (var field in aspects)
        {
            ...
            if (fieldTypeName != null)
            {
                ...
                builder = Utils.Indent(builder, 3);
                builder.AppendLine($"{field.Name}.Initialize<{fieldType}so{fieldTypeName}>(\"{field.Name.Split('.').LastOrDefault()?.Replace("_", "").ToUpper()}\",this, {field.Name}Data);");
            }
            builder = Utils.Indent(builder, 3);
            builder.AppendLine($"Aspects[\"{field.Name.Split('.').LastOrDefault()?.Replace("_", "").ToUpper()}\"] = {field.Name};");
        }
        return builder.ToString();
    }
